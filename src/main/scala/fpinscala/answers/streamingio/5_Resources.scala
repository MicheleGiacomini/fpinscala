...
object Resources:
    ...
    def close: F[Unit] =
      state.modify {
        case State.Open(finalizer, subscopes) =>
          val finalizers = (subscopes.reverseIterator.map(_.close) ++ Iterator(
            finalizer
          )).toList
  ...
  enum Pull[+F[_], +O, +R]:
            ...
            .flatMap { case (newScope, closeAfterUse) =>
              source.step(newScope).attempt.flatMap {
                case Success(Out(scope, hd, tl)) =>
                  F.unit(Out(scope, hd, WithScope(tl, scopeId, returnScopeId)))
    ...
    def mapAccumulate[S, O2](
        init: S
    )(f: (S, O) => (S, O2)): Pull[F, O2, (S, R)] =
      uncons.flatMap {
