...
class RNGSuite extends PropSuite:
  ...
  test("RNG.double3")(genRNG ** genCounter) { case rng ** counter =>
    val isCorrect: ((Double, Double, Double)) => Boolean =
      (d1, d2, d3) =>
        isInInterval(d1) && isInInterval(d2) && isInInterval(
          d3
        ) && d1 != d2 && d1 != d3 && d2 != d3
  ...
  test("RNG.map")(genRNG ** genCounter) { case rng ** counter =>
    assert(
      checkRND(rng, counter, map(int)(_.toString), _.toIntOption.isDefined)
    )
  }
  ...
  test("RNG.map2")(genRNG ** genCounter) { case rng ** counter =>
    ...
    assert(
      checkRND(
        ...
        counter,
        randC,
        (d1, d2) => isInInterval(d1) && isInInterval(d2) && d1 != d2
      )
    )
  }
  ...
  test("RNG.flatMap")(genRNG ** genCounter ** genSmallPosNum) {
    case rng ** counter ** limit =>
      ...
      assert(
        checkRNGNonNegativeLessThan(
          rng,
          counter,
          nonNegativeLessThan(limit),
          limit
        )
      )
  }
  ...
  test("RNG.mapViaFlatMap")(genRNG ** genCounter) { case rng ** counter =>
    assert(
      checkRND(
        rng,
        counter,
        mapViaFlatMap(int)(_.toString),
        _.toIntOption.isDefined
      )
    )
  }
    ...
    def loop(rng: RNG, counter: Int, previousValue: Option[A] = None): Boolean =
      counter < 0 || {
        ...
        if !isCorrect(value) then
          fail(s"The generated value '$value' is invalid.")
        else if previousValue.contains(value) then
          fail(s"The RNG generated two identical values in a row ('$value').")
        else loop(rng2, counter - 1, Some(value))
      }
    loop(rng, counter)

  private def checkRNGUnit[A](
      rng: RNG,
      counter: Int,
      constantValue: A
  ): Boolean =
    ...
    def loop(rng: RNG, counter: Int): Boolean =
      counter < 0 || {
        ...
        if constantValue == value then loop(rng2, counter - 1)
          ...
          fail(
            s"RNG.unit must always return a constant value ('$constantValue')."
          )
      }
    loop(rng, counter)

  private def checkRNGNonNegativeLessThan(
      rng: RNG,
      counter: Int,
      generate: Rand[Int],
      limit: Int
    ...
    def loop(rng: RNG, counter: Int): Boolean =
      counter < 0 || {
        ...
        if 0 <= i && i < limit then loop(rng2, counter - 1)
          ...
          fail(s"The generated value '$i' isn't in the interval [0, $limit).")
      }
    loop(rng, counter)
